...
class TestCustomLoginForm:
    """Test suite for CustomLoginForm."""

    def test_login_form_fields_exist(self):
        """
        Test that login form has required fields.
        
        Why: Ensures the form includes all necessary fields for
        user authentication.
        """
    ...
    def test_login_form_field_customization(self):
        """
        Test that login form fields have Czech labels and placeholders.
        
        Why: Validates that the form is properly localized for Czech users
        with appropriate CSS classes and autocomplete attributes.
        """
        form = CustomLoginForm()
        
        # Check login field customization
        login_field = form.fields['login']
        assert login_field.label == 'Uživatelské jméno nebo e-mail'
        assert login_field.widget.attrs['class'] == 'form-input'
        assert login_field.widget.attrs['placeholder'] == 'vas@email.cz nebo uzivatelskeJmeno'
        assert login_field.widget.attrs['autocomplete'] == 'username'
        
        # Check password field customization
        password_field = form.fields['password']
        assert password_field.label == 'Heslo'
        assert password_field.widget.attrs['class'] == 'form-input'
        assert password_field.widget.attrs['placeholder'] == 'Vaše heslo'
        assert password_field.widget.attrs['autocomplete'] == 'current-password'

    def test_login_form_widget_classes(self):
        """
        Test that form widgets have correct CSS classes.
        
        Why: Ensures consistent styling across the application.
        """
        form = CustomLoginForm()
        
        for field_name in ['login', 'password']:
            widget_class = form.fields[field_name].widget.attrs.get('class')
            assert widget_class == 'form-input'


# ============================================
# CUSTOM SIGNUP FORM TESTS
# ============================================
...
class TestCustomSignupForm:
    """Test suite for CustomSignupForm."""

    def test_signup_form_fields_exist(self):
        """
        Test that signup form has all required fields.
        
        Why: Ensures users can provide all necessary information
    ...
    def test_signup_form_field_customization(self):
        """
        Test that signup form fields have Czech labels and help texts.
        
        Why: Validates proper localization and user guidance during registration.
        """
        form = CustomSignupForm()
        
        # Username
        assert form.fields['username'].label == 'Uživatelské jméno'
        assert 'Minimálně 3 znaky' in form.fields['username'].help_text
        assert form.fields['username'].widget.attrs['placeholder'] == 'uzivatelskeJmeno'
        
        # Email
        assert form.fields['email'].label == 'E-mailová adresa'
        assert 'obnovení hesla' in form.fields['email'].help_text
        assert form.fields['email'].widget.attrs['placeholder'] == 'vas@email.cz'
        
        # Password1
        assert form.fields['password1'].label == 'Heslo'
        assert 'Minimálně 8 znaků' in form.fields['password1'].help_text
        assert form.fields['password1'].widget.attrs['placeholder'] == 'Silné heslo'
        
        # Password2
        assert form.fields['password2'].label == 'Potvrzení hesla'
        assert form.fields['password2'].widget.attrs['placeholder'] == 'Zadejte heslo znovu'

    def test_signup_form_autocomplete_attributes(self):
        """
        Test that form fields have correct autocomplete attributes.
        
        Why: Improves UX by allowing browsers to auto-fill fields correctly.
        """
        form = CustomSignupForm()
    ...
    def test_signup_form_save_method_exists(self):
        """
        Test that save method exists and accepts request parameter.
        
        Why: Ensures compatibility with django-allauth's signup flow.
        """
        # We can't fully test save without a valid request object and session
