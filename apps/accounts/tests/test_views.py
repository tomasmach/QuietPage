...
- AccountDeleteView: delete user and cascade
"""
...
class TestAccountDeleteView:
    ...
    def test_get_account_delete_page(self, client):
        """Test loading the account deletion page."""
        user = UserFactory()
        client.force_login(user)
        
        response = client.get(reverse('accounts:settings-delete'))
        
        assert response.status_code == 200
        assert 'form' in response.context
        assert response.context['active_section'] == 'delete'

    def test_shows_entry_count_in_context(self, client):
        """Test that total entries count is shown in context."""
        user = UserFactory()
        client.force_login(user)
        ...
        assert response.context['total_entries'] == 5

    def test_delete_account_successfully(self, client):
        """Test successful account deletion."""
        user = UserFactory(username='testuser')
        client.force_login(user)
        
        response = client.post(reverse('accounts:settings-delete'), {
            'password': 'testpass123',
            'confirm_text': 'SMAZAT',
        })
    ...
    def test_user_logged_out_after_deletion(self, client):
    ...
    def test_entries_cascade_deleted(self, client):
        """Test that user entries are cascade deleted with account."""
        user = UserFactory()
        client.force_login(user)
        
        # Create entries
        entries = EntryFactory.create_batch(3, user=user)
        entry_ids = [e.id for e in entries]
        
        client.post(reverse('accounts:settings-delete'), {
            'password': 'testpass123',
            'confirm_text': 'SMAZAT',
        })
        
        # Check entries are deleted
    ...
    def test_success_message_displayed(self, client):
        ...
        messages = list(get_messages(response.wsgi_request))
        assert any('trvale smazán' in str(m).lower() for m in messages)
        assert any('testuser' in str(m) for m in messages)
